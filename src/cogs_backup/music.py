...
# bgutil-ytdlp-pot-provider는 yt-dlp 플러그인이므로 직접 import하지 않습니다
# yt-dlp가 자동으로 감지하여 사용합니다
BGUTIL_AVAILABLE = True  # yt-dlp가 자동 감지
logger.info("bgutil-ytdlp-pot-provider는 yt-dlp 플러그인으로 자동 로드됩니다")
...
class YTDLPSource:
    """yt-dlp를 사용한 음악 소스 처리 (자동 PO Token 방식)"""
    ...
    YTDL_OPTIONS = {
        ...
        # PO Token 설정 (YouTube bot detection bypass - 2024년 이후 권장 방식)
        'extractor_args': {
            'youtube': {
                'player_client': ['ios', 'web'],  # ios 우선 사용 (안정적)
                'po_token': None,  # bgutil 또는 환경 변수에서 동적으로 설정됨
                'visitor_data': None,  # 동적으로 설정됨
                'player_skip': ['configs'],
                'skip': ['hls', 'dash'],
            }
        },
    ...
    def __init__(self):
        ...
        # bgutil-ytdlp-pot-provider는 yt-dlp 플러그인으로 자동 작동
        # 설치되어 있으면 yt-dlp가 자동으로 PO Token을 생성합니다
        logger.info("✅ bgutil-ytdlp-pot-provider 사용 (yt-dlp 플러그인)")
        logger.info("   → PO Token 자동 생성 (수동 설정 불필요)")
        
        # 수동 PO Token 설정 (백업용 - bgutil 없을 때만 사용됨)
        po_token = Config.get_youtube_po_token()
        if po_token:
            options['extractor_args']['youtube']['po_token'] = po_token
            logger.info("   → 추가: 수동 PO Token 설정됨")
        
        # Visitor Data 설정
        visitor_data = Config.get_youtube_visitor_data()
        if visitor_data:
            options['extractor_args']['youtube']['visitor_data'] = visitor_data
            logger.info(f"YouTube Visitor Data 설정 완료: {visitor_data[:10]}...")
        ...
        logger.info("yt-dlp 초기화 완료")
        
        try:
            self.ytdl = yt_dlp.YoutubeDL(options)
            logger.info("✅ yt-dlp 초기화 완료 (bgutil 플러그인 자동 감지)")
...
class MusicCog(commands.Cog):
    ...
    async def cleanup_all_players(self):
        """모든 플레이어 정리 (외부 호출용)"""
        logger.info("음악 시스템 정리 - 모든 플레이어 연결 해제 중...")
        player_count = len(self.players)
        
        for guild_id, player in list(self.players.items()):
            try:
                logger.debug(f"길드 {guild_id} 플레이어 정리 중...")
                await player.disconnect()
            except Exception as e:
                logger.error(f"길드 {guild_id} 플레이어 정리 중 오류: {e}")
        
        self.players.clear()
        logger.info(f"음악 시스템 정리 완료 - {player_count}개 플레이어 정리됨")
        
    async def cog_unload(self):
        """Cog 언로드 시 모든 음성 연결 정리"""
        logger.info("음악 시스템 종료 - 모든 연결 정리 중...")
        await self.cleanup_all_players()
        logger.info("음악 시스템 언로드 완료")
