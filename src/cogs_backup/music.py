...
class YTDLPSource:
    ...
    async def extract_info(self, search: str) -> tuple[Optional[Track], Optional[str]]:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"음악 검색 시작 (시도 {attempt + 1}/{max_retries}): {search}"
                )
                ...
                if attempt > 0:
                    ...
                    try:
                        if (
                            ...
                            logger.info(
                                f"재시도 - 새 User-Agent 사용: {new_user_agent[:50]}..."
                            )
                    except Exception as e:
                ...
                if attempt < max_retries - 1:
                    ...
                    if any(
                        ...
                        logger.info(
                            f"재시도 가능한 오류 - {retry_delay * (attempt + 1)}초 후 재시도"
                        )
                        continue
        ...
        return (
            None,
            f"모든 재시도 실패 ({max_retries}회 시도). 잠시 후 다시 시도해주세요.",
        )
    ...
    async def play_track(self, track: Track) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                return (
                    False,
                    "오디오 스트림을 생성할 수 없습니다. 다른 곡을 시도해주세요.",
                )
            ...
            if not self.voice_client.is_playing():
                ...
                return (
                    False,
                    "재생을 시작할 수 없습니다. 스트림에 문제가 있을 수 있습니다.",
                )
...
class MusicCog(commands.Cog):
    ...
    async def play_music(self, interaction: discord.Interaction, 검색어: str):
        ...
        if player.is_playing:
            await interaction.followup.send("이미 노래가 재생중입니다", ephemeral=True)
            return
        ...
        if success:
            ...
            embed.add_field(name="🎤 아티스트", value=track_info.uploader, inline=False)
            ...
            embed = create_error_embed(
                "❌ 재생 실패", play_error or "음악을 재생할 수 없습니다."
            )
            message = await interaction.followup.send(embed=embed)
            asyncio.create_task(self._delete_message_after_delay(message, 10))
