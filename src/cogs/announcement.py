...
class AnnouncementCog(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.rules_file_path = Path(__file__).resolve().parent.parent / "data" / "rules.json"
    ...
    async def rules(
        ...
        if not isinstance(target_channel_raw, discord.TextChannel):
            await interaction.followup.send("❌ 텍스트 채널에서만 규칙을 게시할 수 있습니다.", ephemeral=True)
            return
        ...
        try:
            async for message in channel.history(limit=limit):
                if (message.author == self.bot.user and 
                    ...
                    if message.id == PERSISTENT_RULE_MESSAGE_ID or getattr(message, "_siri_skip_cleanup", False):
                        continue
