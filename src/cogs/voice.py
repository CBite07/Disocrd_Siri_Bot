...
class VoiceCog(commands.Cog):
    ...
    async def generate_tts(self, text: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"TTS 파일 생성 완료: {temp_file.name} (크기: {file_size} bytes)"
            )
    ...
    @app_commands.describe(활성화="자동 참여 모드 활성화 여부 (ON/OFF)")
    ...
    async def auto_join(
        self, interaction: discord.Interaction, 활성화: app_commands.Choice[int]
    ):
        """
        자동 참여 모드 설정 (관리자 전용)
    ...
    @app_commands.command(name="시리야", description="시리를 음성 채널로 불러옵니다")
    async def join_voice(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.info(
                f"음성 채널 참여: {channel.name} (길드: {interaction.guild.name})"
            )
    ...
    @app_commands.command(name="퇴장해", description="시리를 음성 채널에서 내보냅니다")
    async def leave_voice(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.info(
                f"음성 채널 퇴장: {channel_name} (길드: {interaction.guild.name})"
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if len(tts_text.strip()) < 2:
                logger.debug(
                    f"TTS 건너뜀: 너무 짧은 메시지 (처리된 텍스트: {tts_text})"
                )
                return
            ...
            logger.info(
                f"TTS 메시지 읽음: {message.author.display_name} - {tts_text[:50]}"
            )
    ...
    async def on_voice_state_update(
        ...
        if after.channel and not before.channel and auto_join_enabled:
            ...
            try:
                ...
                try:
                    await self.play_tts(
                        guild.voice_client, f"{member.display_name}님, 안녕하세요!"
                    )
                except Exception as e:
                    logger.error(f"자동 참여 TTS 재생 실패: {e}")

                logger.info(
                    f"자동 참여: {after.channel.name} (사용자: {member.display_name})"
                )
