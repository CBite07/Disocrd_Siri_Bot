...
class AdminCog(commands.Cog):
        ...
        try:
            ...
            if roles_to_remove:
                await member.remove_roles(
                    *roles_to_remove, reason="레벨 변경으로 인한 역할 변경"
                )
    ...
    @app_commands.command(
        name="레벨설정", description="특정 유저의 레벨을 설정합니다 (관리자 전용)"
    )
    @app_commands.describe(유저="레벨을 설정할 대상 유저", 레벨="설정할 레벨 (1~100)")
    async def set_level(
        self, interaction: discord.Interaction, 유저: discord.Member, 레벨: int
    ):
        """레벨 설정"""
        # 길드 컨텍스트 확인
        ...
        if not (1 <= 레벨 <= 100):
            embed = create_error_embed(
                "❌ 잘못된 값", "레벨은 1부터 100까지만 설정할 수 있습니다."
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        # XP 업데이트
        success = await self.bot.db.update_user_xp(
            유저.id, interaction.guild.id, xp_difference
        )
    ...
    @app_commands.command(
        name="데이터초기화",
        description="특정 유저의 데이터를 초기화합니다 (관리자 전용)",
    )
    ...
    @app_commands.command(
        name="명령어목록", description="등록된 모든 명령어를 확인합니다 (개발자용)"
    )
...
class DataResetConfirmView(discord.ui.View):
    ...
    async def confirm_reset(
        ...
        if interaction.guild is None:
            await interaction.followup.send(
                "❌ 이 작업은 서버에서만 가능합니다.", ephemeral=True
            )
            return
        success = await self.database.reset_user_data(
            self.target_user.id, interaction.guild.id
        )

        # 레벨 역할 제거
        roles_removed = await self.remove_level_roles(
            interaction.guild, self.target_user
        )
    ...
    async def remove_level_roles(
        ...
        try:
            ...
            if roles_to_remove:
                await member.remove_roles(
                    *roles_to_remove, reason="데이터 초기화로 인한 레벨 역할 제거"
                )
                removed_count = len(roles_to_remove)
                logger.info(
                    f"{member.display_name}에게서 {removed_count}개의 레벨 역할 제거 완료"
                )
    ...
    async def cancel_reset(
        ...
        """데이터 초기화 취소"""
        embed = create_error_embed("❌ 취소됨", "데이터 초기화가 취소되었습니다.")
