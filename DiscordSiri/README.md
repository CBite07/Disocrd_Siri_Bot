...
### 2.6. Edge-TTS 음성 기능 (Phase 6)

> 사용자 스토리:
> 서버 멤버로서, 시리 봇을 음성 채널로 불러와 음성 안내를 듣고 싶다. 관리자는 자동 참여 기능을 켜서 누군가 입장할 때마다 시리가 자동으로 인사하길 원한다.

**요구사항:**

-   **명령어**:
    -   `/자동참여 [활성화: ON/OFF]` (관리자 전용)
        -   자동 참여 모드를 활성화/비활성화
        -   서버별로 독립적인 설정 관리
        -   에페머럴 메시지로 응답: "자동 참여 모드를 활성화/비활성화할게요!"
    -   `/시리야`
        -   사용자가 있는 음성 채널에 봇 입장
        -   임베드 메시지: "🎤 시리 참여 - 지금 바로 들어갈게요!"
        -   TTS 인사: "안녕하세요! 시리가 왔어요!"
    -   `/퇴장해`
        -   음성 채널에서 봇 퇴장
        -   임베드 메시지: "👋 시리 퇴장 - 좋아요, 이만 물러날게요!"
        -   TTS 작별 인사: "안녕히 계세요!"

**자동 기능:**

-   **자동 입장**: 자동 참여 모드 활성화 시, 누군가 음성 채널 입장 시 봇이 자동으로 따라 들어가 "{사용자명}님, 안녕하세요!" 인사
-   **자동 퇴장**: 봇을 제외한 사용자가 없으면 자동으로 채널에서 나감

**의존성:**

-   Python 패키지: `edge-tts` (Microsoft Edge TTS)
-   시스템 패키지: `ffmpeg` (오디오 재생에 필수)

**기술 스펙:**

-   **TTS 엔진**: Microsoft Edge TTS (고품질 한국어 음성)
-   **음성**: `ko-KR-SunHiNeural` (한국어 여성 음성)
-   **임시 파일**: 시스템 임시 디렉토리에 생성 후 재생 완료 시 자동 삭제

**상세 가이드:**

-   자세한 사용법과 문제 해결은 [VOICE_FEATURE.md](./VOICE_FEATURE.md) 참조
